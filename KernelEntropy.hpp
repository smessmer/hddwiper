...
{
public:
  static const Data getEntropy(const unsigned int size, boost::function<void (unsigned int)> callback=DummyCallback());
};

inline const Data KernelEntropy::getEntropy(const unsigned int size, boost::function<void (unsigned int)> callback)
{
  Data result(size);
