...
int main(int argc, char *argv[])
{
  ...
  while(true)
  {
    ...
    write(random, output);
    ++blocks;
    const double mb_per_sec=static_cast<double>(RC4Streamgenerator::BLOCKSIZE)/1024/1024/time.getSecondsPassed();
    cout << "\rWritten: "<<blocks<<" blocks ("
	<<static_cast<double>(blocks)*(static_cast<double>(RC4Streamgenerator::BLOCKSIZE)/1024/1024/1024)<<" GB) "
	<< "Speed: "<<mb_per_sec << "MB/s                    " << flush;
